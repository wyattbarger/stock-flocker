...
router.post("/register", async (req, res) => {
  try {
    ...
    req.session.save(() => {
      // Similar to the block in the /login POST, however in this case we are now logging a user in after successful registration, and storing the express-session.
...
router.post("/login", async (req, res) => {
  // Added a POST route for logging in.
  try {
    // Added try-catch to handle errors.
    const userInfo = await User.findOne({
      where: { username: req.body.username },
    }); // Added code to find one user based on their username, this has been given an unique property in the model
    if (!userInfo) {
      // Added a condition to make the the user info matches what is on record based on the the username, which was defined as unique by andmell in models/user.js.
      res
        .status(400)
        .json({ message: "Wrong username or password, please try again!" }); // Tell the user they entered some of their info incorrect and return.
      return;
    }
    ...
    req.session.save(() => {
      // If the users enters the correct info, save the session
